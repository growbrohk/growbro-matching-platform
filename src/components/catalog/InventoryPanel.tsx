...
import { Loader2, RefreshCw, ChevronDown, ChevronRight, ChevronsDown, Upload } from 'lucide-react';
import { getVariantHierarchy, getVariantOptionValue } from '@/lib/utils/variant-parser';
...
export function InventoryPanel({ orgId }: InventoryPanelProps) {
  const { currentOrg, user } = useAuth();
  const { toast } = useToast();

  // Use orgId prop, fallback to currentOrg?.id if needed
  ...
  const [viewMode, setViewMode] = useState<'grouped' | 'list'>(() => {
    if (!effectiveOrgId) return 'grouped';
    const saved = localStorage.getItem(`inventory:viewMode:${effectiveOrgId}`);
  ...
  useEffect(() => {
    if (!effectiveOrgId) return;
    localStorage.setItem(`inventory:viewMode:${effectiveOrgId}`, viewMode);
  }, [viewMode, effectiveOrgId]);
  ...
  const getBulkUpdatePreview = () => {
    if (!bulkUpdateCsv.trim() || !effectiveOrgId) {
  ...
  const applyBulkUpdate = async () => {
    if (!effectiveOrgId || !effectiveUser || selectedWarehouseId === 'all') return;
        ...
        <CardContent className="p-4 md:p-6">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <Button variant="outline" onClick={() => setBulkUpdateOpen(true)} size="sm">
