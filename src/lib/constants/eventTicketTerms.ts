/**
 * Default Event Ticket Terms & Conditions
 * Used as preset in create/edit event form. Organizers can customize per event.
 */
export const DEFAULT_EVENT_TICKET_TERMS = `Event Ticket Terms & Conditions

By purchasing or using a ticket for this event, you agree to the following terms and conditions:

1. Ticket Purchase

All ticket sales are final unless otherwise stated. Tickets are non-refundable except in the case of event cancellation by the organizer.

2. Event Changes

The organizer reserves the right to modify the event details, including schedule, venue, route, activities, or program if necessary due to weather, safety concerns, or unforeseen circumstances.

3. Event Cancellation & Weather Policy
Standard Policy: If the event is cancelled by the organizer for administrative reasons, participants will receive a full refund of the ticket price. Service or payment processing fees may not be refundable depending on the payment provider.

Extreme Weather: The event will only be cancelled or rescheduled if the Hong Kong Observatory issues a Tropical Cyclone Warning Signal No. 8 (or above) or a Black Rainstorm Warning that is in force two (2) hours before the scheduled start time.

Other Conditions: For all other weather signals (including Typhoon Signal No. 3, Amber, or Red Rainstorm warnings), the event will proceed as planned unless the organizer deems the conditions unsafe on-site.

No Refunds for Weather: If the event is cancelled or rescheduled due to the extreme weather signals mentioned above (T8 or Black Rain), tickets remain non-refundable, but the organizer may, at its discretion, provide an alternative date or virtual participation option.

4. Participant Responsibility

Participants are responsible for ensuring they are physically fit and capable of participating in the event. Participation is at the participant's own risk.

5. Liability Waiver

The organizer shall not be held liable for any injury, loss, damage, or personal belongings during the event, except where required by law.

6. Photography & Media

By attending the event, participants consent to photography and video recording that may be used for promotional or documentation purposes by the organizer.

7. Ticket Usage

Each ticket is valid for one participant only and may not be duplicated or resold without permission from the organizer.

8. Code of Conduct

Participants are expected to behave respectfully toward other participants, organizers, and venues. The organizer reserves the right to remove any participant who behaves inappropriately without refund.

9. Refunds & Transfers

Unless otherwise stated, tickets are non-refundable. Ticket transfers may be permitted before the event at the organizer's discretion.

10. Acceptance of Terms

By purchasing or using a ticket, participants confirm they have read and agreed to these terms and conditions.`;

/** Default label for optional marketing opt-in checkbox at checkout */
export const DEFAULT_MARKETING_OPT_IN_LABEL =
  'I would like to receive the latest news and promotions';
