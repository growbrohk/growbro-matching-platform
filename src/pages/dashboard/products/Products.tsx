  ...
  const fetchProductStocksAndVariations = useCallback(async (products: Product[]) => {
    ...
    if (allVariationIds.length > 0) {
      ...
      for (const chunk of chunks) {
        ...
        if (error) {
          console.error('Error fetching variation inventory in Products page:', error);
        } else if (data) {
      ...
      if (allVariationInvData.length > 0) {
        allVariationInvData.forEach((inv) => {
          ...
          if (!variationInventoryMap[variationId]) {
            variationInventoryMap[variationId] = {};
          }
          variationInventoryMap[variationId][inv.inventory_location_id] = inv.stock_quantity || 0;
        });
      }
    }
