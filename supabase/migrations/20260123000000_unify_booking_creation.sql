-- Migration: Unify booking creation - ALWAYS create orders + tickets
-- Fixes:
-- 1. Per-Ticket incognito mode: Allow orders when attendees[0] has email
-- 2. Per-Ticket mode: Use Attendee 1 info as order contact
-- 3. Ensure ALL bookings create both orders and tickets
...
BEGIN
  ...
  -- ============================================================================
  -- STEP 6: CREATE ORDER ITEMS AND TICKETS (ALWAYS)
  -- ============================================================================
  v_attendee_index := 0;
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_order_lines)
  LOOP
