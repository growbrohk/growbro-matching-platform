...
BEGIN
  ...
  -- ============================================================================
  -- ALWAYS prefer p_buyer_* parameters (Order Contact) as the source of truth
  -- Fallback to Attendee 1 only if p_buyer_* is not provided (backward compatibility)
  -- This ensures orders.buyer_email is stable and matches what the UI collected as Order Contact
  
  IF p_attendees IS NOT NULL AND jsonb_array_length(p_attendees) > 0 THEN
    -- Per-Ticket mode: Prefer p_buyer_* (Order Contact), fallback to Attendee 1
    v_first_attendee := p_attendees->0;
    v_order_first_name := COALESCE(NULLIF(p_buyer_first_name, ''), v_first_attendee->>'first_name');
    v_order_last_name := COALESCE(NULLIF(p_buyer_last_name, ''), v_first_attendee->>'last_name');
    v_order_email := COALESCE(NULLIF(p_buyer_email, ''), v_first_attendee->>'email');
    v_order_phone := COALESCE(NULLIF(p_buyer_phone, ''), v_first_attendee->>'phone');
  ELSE
...
ORDER CONTACT (buyer_email) DETERMINATION:
- ALWAYS prefers p_buyer_email (Order Contact) as source of truth
- For Per-Ticket mode: Uses p_buyer_* if provided, falls back to Attendee 1 only if p_buyer_* is empty
- For Primary Booker mode: Uses p_buyer_* directly
- This ensures orders.buyer_email is stable and matches UI-collected Order Contact email
- Prevents "Unauthorized: Email does not match order buyer_email" errors in receipt submission
