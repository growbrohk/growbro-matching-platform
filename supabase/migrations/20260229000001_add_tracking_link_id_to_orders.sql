-- Migration: Add tracking_link_id parameter to create_event_booking function
-- Enables orders to be attributed to tracking links via tid parameter

-- Drop existing function
DROP FUNCTION IF EXISTS create_event_booking(uuid, jsonb, uuid, text, text, text, text, text, jsonb) CASCADE;

-- Recreate function with tracking_link_id parameter
CREATE OR REPLACE FUNCTION create_event_booking(
  ...
  p_tracking_link_id UUID DEFAULT NULL -- Optional tracking link ID from tid parameter
)
RETURNS UUID
SECURITY DEFINER
...
BEGIN
  ...
  -- ============================================================================
  -- STEP 3: VALIDATE TRACKING LINK (if provided)
  -- ============================================================================
  IF p_tracking_link_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM tracking_links WHERE id = p_tracking_link_id AND is_active = true) THEN
      -- Invalid tracking link, but don't fail order creation - just log warning
      -- Set to NULL to ignore invalid tracking_link_id
      RAISE WARNING 'Invalid or inactive tracking_link_id: %, ignoring', p_tracking_link_id;
      -- Continue without tracking_link_id
    END IF;
  END IF;

  -- ============================================================================
  -- STEP 4: COMPUTE TOTAL AMOUNT (SERVER-SIDE ONLY)
  -- ============================================================================
  ...
  -- ============================================================================
  -- STEP 7: CREATE ORDER (using SERVER-COMPUTED total_amount)
  -- ============================================================================
  INSERT INTO orders (
    event_id,
    buyer_user_id,
    ...
    fulfillment_status,
    order_no,
    tracking_link_id,  -- ✅ Optional tracking link attribution
  ...
  VALUES (
    ...
    v_order_no,
    p_tracking_link_id,  -- ✅ Optional tracking link attribution
  ...
  -- ============================================================================
  -- STEP 8: CREATE ORDER ITEMS AND TICKETS (using SERVER-COMPUTED prices)
  -- ============================================================================
  v_attendee_index := 0;
  ...
  LOOP
    ...
    LOOP
      ...
      ELSE
        -- Primary Booker mode: Use order contact info (or NULL if not provided)
        INSERT INTO tickets (
          order_id,
          order_item_id,
          ticket_type_id,
...
-- Set search_path for function
ALTER FUNCTION create_event_booking(uuid,jsonb,uuid,text,text,text,text,text,jsonb,uuid) SET search_path = public;

COMMENT ON FUNCTION create_event_booking IS 'Creates an event booking (order) with server-computed amounts and optional tracking link attribution. Returns order UUID.';
