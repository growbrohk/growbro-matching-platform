-- Add valid_for_days to get_ticket_types_with_remaining RPC
-- Required for multi-day events: public event page uses this to display correct
-- Day 1 / Day 2 / Both times per ticket type via formatTicketTypeDateTime
...
COMMENT ON FUNCTION get_ticket_types_with_remaining(UUID) IS 'Returns ticket types for an event with calculated remaining_count (quota - sold tickets with status valid/scanned). Includes valid_for_days for multi-day event display.';
