-- Migration: Product order confirmation email
-- 1. Restrict existing event trigger to event orders only (preserves event flow)
-- 2. Add new trigger + function for product order confirmation emails
--
-- Event flow: UNCHANGED - only the WHEN clause is narrowed to order_type = 'event'
-- Product flow: NEW - sends confirmation email when product order is confirmed
--
-- ============================================================================
-- STEP A: Restrict event trigger to event orders only
-- ============================================================================
...
CREATE TRIGGER trigger_order_confirmed_send_email
  ...
  WHEN (
    ...
    AND (NEW.order_type = 'event' OR NEW.order_type IS NULL)
  )
  EXECUTE FUNCTION public.on_order_confirmed_send_email();

-- ============================================================================
-- STEP B: Create product order confirmation email trigger and function
-- ============================================================================
...
-- Create trigger for product orders (drop first for idempotency)
DROP TRIGGER IF EXISTS trigger_product_order_confirmed_send_email ON public.orders;

CREATE TRIGGER trigger_product_order_confirmed_send_email
  AFTER UPDATE OF fulfillment_status ON public.orders
  FOR EACH ROW
  WHEN (
    OLD.fulfillment_status IS DISTINCT FROM NEW.fulfillment_status
  ...
  EXECUTE FUNCTION public.on_product_order_confirmed_send_email();
