-- Host opt-in: show_remaining_stock on event_addon_products
-- get_event_addons_for_checkout: per-variant stock_remaining (null when flag off; 0+ when flag on, default warehouse)

ALTER TABLE event_addon_products
ADD COLUMN IF NOT EXISTS show_remaining_stock BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN event_addon_products.show_remaining_stock IS 'When true, checkout shows remaining inventory per variant (default org warehouse) and create_event_booking enforces stock.';
...
COMMENT ON FUNCTION get_event_addons_for_checkout IS 'Returns add-on products for event checkout (image URL, optional per-variant stock_remaining when show_remaining_stock). Public for published events.';
