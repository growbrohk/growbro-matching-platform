-- Enforce add-on inventory when event_addon_products.show_remaining_stock is true
-- Same default warehouse ordering as deduct_addon_inventory_on_payment
...
COMMENT ON FUNCTION create_event_booking IS 'Creates event booking with tickets and optional add-on products. Validates add-on inventory when show_remaining_stock. Order lines may include ticket_type_access_variant_id. All amounts computed server-side.';
